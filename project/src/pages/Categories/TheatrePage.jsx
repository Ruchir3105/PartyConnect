...
const TheatrePage = () => {
  ...
  useEffect(() => {
    // Fetch theatre events from the API
    const fetchTheatreEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/theatre');
        setTheatreEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching theatre events');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white">Loading theatre events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Theatre Events</h1>
      <p className="text-lg mb-4">Enjoy plays, musicals, and live stage performances!</p>
