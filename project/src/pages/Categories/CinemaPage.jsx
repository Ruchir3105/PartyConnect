...
const CinemaPage = () => {
  const [cinemaEvents, setCinemaEvents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  useEffect(() => {
    // Fetch cinema events from the API
    const fetchCinemaEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/cinema');
        setCinemaEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching cinema events');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white">Loading cinema events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Cinema Events</h1>
      <p className="text-lg mb-4">Experience the magic of movies with premieres, festivals, and special screenings!</p>
