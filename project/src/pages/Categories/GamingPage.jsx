...
const GamingPage = () => {
  ...
  useEffect(() => {
    // Fetch gaming events from the API
    const fetchGamingEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/gaming');
        setGamingEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching gaming events');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white">Loading gaming events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Gaming Events</h1>
      <p className="text-lg mb-4">Join tournaments, LAN parties, and esports competitions!</p>
