...
const ComedyPage = () => {
  ...
  useEffect(() => {
    // Fetch comedy events from the API
    const fetchComedyEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/comedy');
        setComedyEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching comedy events');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white">Loading comedy events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Comedy Events</h1>
      <p className="text-lg mb-4">Laugh out loud with stand-up specials, improv shows, and comedy nights!</p>
