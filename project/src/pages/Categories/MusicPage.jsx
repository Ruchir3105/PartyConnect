...
const MusicPage = () => {
  ...
  useEffect(() => {
    // Fetch music events from the API
    const fetchMusicEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/music');
        setMusicEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching music events');
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="text-white">Loading music events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Music Events</h1>
      <p className="text-lg mb-4">Experience live concerts, festivals, and musical performances!</p>
