...
const FoodDrinkPage = () => {
  const [foodDrinkEvents, setFoodDrinkEvents] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const navigate = useNavigate();

  useEffect(() => {
    // Fetch food & drink events from the API
    const fetchFoodDrinkEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8080/events/fooddrink');
        setFoodDrinkEvents(response.data);
        setLoading(false);
      } catch (err) {
        setError('Error fetching food & drink events');
        setLoading(false);
      }
    };

    fetchFoodDrinkEvents();
  }, []);
  ...
  if (loading) {
    return (
      <div className="text-white">Loading food & drink events...</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-bold mb-8">Food & Drink Events</h1>
      <p className="text-lg mb-4">Savor culinary experiences, tastings, and food festivals!</p>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {foodDrinkEvents.map(event => (
